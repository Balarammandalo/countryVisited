...
class App extends Component {
  ...
  onVisitCountry = activeId => {
    const {countryList} = this.state

    const visitCountry = countryList.map(eachCountry => {
      if (eachCountry.id === activeId) {
        const updateCountry = {
          ...countryList,
  ...
  renderVisitedCountries = () => {
    ...
    const visitedCountry = countryList.filter(
      eachCountry => eachCountry.isVisited === true,
    )
    const isEmpty = visitedCountry.length > 0

    return (
      <>
        {isEmpty ? (
          <ul className='visited-list'>
            {visitedCountry.map(eachCountry => (
              <VisitedCountries
                key={eachCountry.id}
                visitedCountryList={eachCountry}
                onClickRemove={this.onClickRemoveCountry}
              />
            ))}
          </ul>
        ) : (
  ...
  render() {
    return (
      <div className='country-container'>
        ...
        <h1 className='coutnry-para'>Visited Countries</h1>
